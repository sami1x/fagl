          ...
          sendEmailVerification(user)
            .then(() => {
              signOut(auth)
                ...
                .catch((error) => {
                  console.log(
                    "email sent successfully but error occured logging out",
                    error,
                  );
                });
            })
            .catch(() => {
              signOut(auth)
                ...
                .catch((error) => {
                  console.log(
                    "error sending email and logging out user",
                    error,
                  );
                });
              setErrorMessage(
                "Email not verified, check your inbox or try again*",
              );
            });
        } else {
          router.push("home");
      ...
      <View className="flex-1 items-center justify-center bg-[url('/assets/main-bg.png')]">
        ...
        >
          ...
          <View className="mx-4 flex w-full max-w-[350px]  flex-col items-center">
            ...
            <Text
              onPress={() => {
                router.push("signin/sendEmail");
              }}
              className=" text-right  w-full text-base font-bold text-white underline"
            >
              Forgot Password?
            </Text>
